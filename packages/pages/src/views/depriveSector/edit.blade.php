...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>Deprive Sector Lending Management</h1>
            ...
            <div class="card">
              <div class="card-body">
                <div class="container-fluid d-flex justify-content-center mb-3">
                  <h5 class="input-title">Edit Deprive Sector Lending</h5>
              </div>

                <!-- Vertical Form -->
                <form class="row g-3 needs-validation" method="post" action="{{route('deprive_sector.update',$depriveSector->id)}}" novalidate >
                    @csrf
                    @method('PUT')
                    <div class="container-fluid p-2 input-container1">
                                ...
                                <div class="input-group has-validation">
                                    <select id="interest-head" name="interest_head_id" class="form-select" required>
                                        ...
                                        @foreach ($interestheads as $data)
                                            <option value="{{$data->id}}" {{$depriveSector->interest_head_id==$data->id?'selected':''}}>{{$data->title}}</option>
                                        @endforeach
                                    </select>
                        ...
                        <table class="table" id="table1">
                            ...
                            @if (is_array($depriveSector) || is_object($depriveSector))
