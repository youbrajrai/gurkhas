                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="container-fluid d-flex justify-content-center mb-3">
                                <h5 class="input-title">Edit Charges</h5>
                            </div>

                            <!-- Vertical Form -->
                            <form class="row g-3 needs-validation" method="post" action="{{route('charge.update',$item->id)}}" novalidate>
                                @csrf
                                @method('PUT')
                                        ...
                                        <div class="input-group has-validation">
                                            <select id="charge-type" name="charge_type_id" class="form-select" required>
                                                ...
                                                @foreach($variables['chargetype'] as $data)
                                                    <option value="{{$data->id}}" {{$item->chargetype->contains($data->id)?'selected':''}}>{{ $data->title }}</option>
                                                @endforeach
                                            </select>
                                            @error('chargetype_id')
                                ...
                                <div class="container-fluid p-2 input-container2">
                                    <div class="row">
                                        <div class="col-4">
                                            ...
                                            <div class="input-group has-validation">
                                                <input type="text" class="form-control" id="charge-title"value="{{old('title',$item->title)}}" name="title"
                                        ...
                                        <div class="col-4">
                                            ...
                                            <div class="input-group has-validation">
                                                <input type="text" class="form-control" id="rate" value="{{old('rate',$item->rate)}}" name="rate"
