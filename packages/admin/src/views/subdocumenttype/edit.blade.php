            ...
            <div class="card">
              <div class="card-body">
                <div class="container-fluid d-flex justify-content-center mb-3">
                  <h5 class="input-title">Edit Sub Document Types</h5>
              </div>

                <!-- Vertical Form -->
                <form class="row g-3 needs-validation" method="post" action="{{route('subdocumenttype.update',$item->id)}}" novalidate >
                    @csrf
                    @method('PUT')
                  <div class="container-fluid p-2 input-container1">
                    <div class="col-4">
                      ...
                      <div class="input-group has-validation">
                          <input type="text" class="form-control" id="document-type" name="title" value="{{old('title',$item->title)}}" placeholder="Sub Document Type*" required>
                          @error('title')
                        ...
                        <div class="input-group has-validation">
                            <select id="document-type" name="document_type_id" class="form-select" required>
                                ...
                                @foreach ($variables['documenttype'] as $data)
                                    <option value="{{$data->id}}" {{$item->document_type_id == $data->id?'selected':''}}>{{$data->title}}</option>
                                @endforeach
                            </select>
